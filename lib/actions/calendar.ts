...
// Helper type for the User query result
type UserWithTokens = {
  _id: string;
  connectedAccounts: ConnectedAccountWithTokens[];
};

// Helper type for the Booking query result
type BookingData = {
  googleEventId: string | null;
  host: {
    // Based on your code usage in cancelBooking, this seems to expect a single account
    // If this is actually an array in your schema, you might need to adjust your query or logic
    connectedAccounts: ConnectedAccountWithTokens;
  } | null;
};
...
export async function getUserConnectedAccountsCount(): Promise<number> {
  ...
  // FIXED: Added <UserWithTokens> generic
  const user = await client.fetch<UserWithTokens>(USER_WITH_TOKENS_QUERY, {
    clerkId: userId,
  });
...
export async function cancelBooking(bookingId: string): Promise<void> {
  ...
  // FIXED: Added <BookingData> generic
  const booking = await client.fetch<BookingData>(
    BOOKING_WITH_HOST_CALENDAR_QUERY,
    {
      bookingId,
    }
  );
